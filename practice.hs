...
guardarParqueadero parqueadero = do
    resultado <- reintentar 5 (withFile "parqueadero.txt" WriteMode $ \h -> do
...
cargarParqueadero = do
    ...
    case resultado of
        Left ex -> do
            ...
            return []
        Right contenido -> do
            -- Forzar la evaluación del contenido para evitar problemas de archivo ocupado
            evaluate (force (length contenido))
...
leerParqueadero = do
    ...
    where
        ...
        parsearVehiculo linea = case words (map (\c -> if c == ',' then ' ' else c) linea) of
...
-- Función para listar los vehículos presentes en el parqueadero
listarVehiculos :: [Vehiculo] -> IO ()
listarVehiculos parqueadero = do
    putStrLn "Mostrando lista de vehículos en el parqueadero:"
    mapM_ (\v -> putStrLn $ "Placa: " ++ placa v ++ ", Entrada: " ++ show (entrada v) ++ ", Salida: " ++ show (salida v)) parqueadero
